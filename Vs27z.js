...
// Function to detect active markers based on certain criteria and fetch data for them
function detectAndFetchDataForActiveMarkers() {
    // Example criteria for detecting active markers; this should be adapted based on actual use case
    const activeMarkers = markers.filter(marker => marker.getVisible());
    fetchAndProcessCSVForActiveMarkers(activeMarkers); // Assuming this function will handle CSV data fetching
}
...
function fetchAndProcessCSVForActiveMarkers(activeMarkers) {
    ...
    .then(csvData => {
        ...
        updateSidebarWithMarkerData(processedData, activeMarkers); // Now passing activeMarkers for more targeted processing
    })
...
function processCSVData(csvData) {
    // Convert CSV data to JSON or a structured JavaScript object for easier handling
    // This example uses a simple CSV parsing approach; for complex data, consider using a library like PapaParse
    ...
    return result; // This is an array of objects, each object representing a row from the CSV
}

// Function to dynamically update the sidebar with data from active markers

function updateSidebarWithMarkerData(processedData) {
    const sidebar = document.getElementById('sidebar'); // Assuming you have a sidebar element with this ID
    sidebar.innerHTML = ''; // Clear existing sidebar content

    processedData.forEach(data => {
        const infoCard = document.createElement('div');
        infoCard.className = 'sidebar-info-card'; // Use a class to style the card

        // Construct the HTML content for each card. Adjust according to your data structure
        infoCard.innerHTML = `
            <h3>${data.popup_header}</h3>
            <p>${data.Description}</p>
            <img src="${data.popupimage_url}" alt="Image description" style="width: 100%;"> // Adjust image styling as needed
            <a href="#" onclick="copyAddress('${data.COPY_ADDRESS}')">Copy Address</a>
            <button onclick="calculateAndDisplayRoute('${data.startLocation}', '${data.endLocation}')">Get Directions</button>
        `;
...
let csvUrl = 'https://raw.githubusercontent.com/checomoandas/noblenomad/main/BsAsPins.csv';
...
function loadCsvData(url) {
    ...
    .then(response => response.text())
    .then(csvText => {
        const data = parseCsv(csvText); // Implement or use a library to parse CSV
        data.forEach(row => {
            // For each row, initialize and add a marker to the map
            // Example: addMarker({lat: parseFloat(row.lat), lng: parseFloat(row.lng), title: row.title});
            // Update sidebar content
            // Example: updateSidebar(row);
        });
    })
...
// Example function to add a marker to the map
function addMarker(markerOptions) {
    const marker = new google.maps.Marker({
        map: map,
        position: { lat: markerOptions.lat, lng: markerOptions.lng },
        title: markerOptions.title,
    });
    markers.push(marker); // Assuming 'markers' is an array holding all markers

    // Optional: Add click listener to marker for updating sidebar
    marker.addListener('click', () => {
        updateSidebar(markerOptions);
    });
}

// Example function to update the sidebar with data from the clicked marker or CSV row
function updateSidebar(data) {
    const sidebar = document.getElementById('sidebar');
    sidebar.innerHTML = `<h1>${data.title}</h1><p>${data.description}</p>`;
}
...
function loadAndProcessCsvData(url) {
    ...
    .then(response => response.text())
    .then(csvText => {
        const processedData = processCSVData(csvText); // Use the existing function
        processedData.forEach(dataRow => {
            addMarker(dataRow); // Adjust this call based on processed data structure
            // Consider updating the sidebar here if necessary
        });
    })
    .catch(error => console.error('Error loading or processing CSV data:', error));
}

// Reminder to adjust addMarker and any other relevant functions to match the data structure
