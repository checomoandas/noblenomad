...
function initKMLLayers() {
    kmlUrls.forEach((url, index) => {
        ...
        google.maps.event.addListener(kmlLayers[index], 'click', function(event) {
            // Close any currently open InfoWindows (including default KML popups)
            if (currentInfowindow) {
                currentInfowindow.close();
            }

            // Delay the opening of the custom InfoWindow
            setTimeout(() => {
                let contentString = `
                <div style="width:250px; height: auto; overflow: auto;">
                    <h1>${event.featureData.name}</h1>
                    <p>${event.featureData.description}</p>
