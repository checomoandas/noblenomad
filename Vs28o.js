...
// Placeholder function to parse CSV data
function parseCSV(csvFileContent) {
    // Assuming the CSV file is simple and each row contains comma-separated values
    // Convert CSV file content to array of objects
...
function updateSidebarWithActiveMarkers() {
    let activeMarkerData = []; // Array to store data for active markers

    // Iterate over markers to check which ones are active
    markers.forEach(marker => {
        if (marker.getMap() !== null) { // Check if the marker is displayed on the map
            // Fetch data from CSV for this marker
            // Assuming each marker has an attribute 'csvRowId' that links to its data in the CSV
            let markerData = parseCSV(csvFileContent).find(row => row.id === marker.csvRowId);
            if (markerData) {
                activeMarkerData.push(markerData);
            }
        }
    });

    // Call populateSidebar with the active marker data
    populateSidebar(activeMarkerData);
}

        }
    });

    // Update the sidebar with active marker data
    populateSidebar(activeMarkerData);
}

function populateSidebar(activeData) {
    let sidebar = document.getElementById('sidebar');
    sidebar.innerHTML = ''; // Clear existing content
    ...
    // Create and append info cards to the sidebar for each active marker
    activeMarkerData.forEach((data, index) => {
        let card = document.createElement('div');
        
function createCardHtml(data) {
    return '<div class="sidebar-info-card">' +
            '<h3>' + data.popup_header + '</h3>' +
            '<img src="' + data.popupimage_url + '" alt="Image">' +
            '<p>' + data.Description + '</p>' +
            '<a href="#" onclick="copyAddress(\'' + data.COPY_ADDRESS + '\')">Copy Address</a>' +
            '<button onclick="calculateAndDisplayRoute(\'' + data.COPY_ADDRESS + '\')">Get Directions</button>' +
           '</div>';
}
...
activeMarkerData.forEach((data, index) => {
    ...
    if (index % 2 === 0) card.className += ' sidebar-card-first-in-row';
...
function updateSidebarWithActiveMarkers() {
    ...
    activeMarkerData.forEach((data, index) => {
        let card = document.createElement('div');
        card.innerHTML = \`<div class="sidebar-info-card">
            <h3>${data.popup_header}</h3>
            <img src="${data.popupimage_url}" alt="Image">
            <p>${data.Description}</p>
            <a href="#" onclick="copyAddress('${data.COPY_ADDRESS}')">Copy Address</a>
