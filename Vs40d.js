...
function updateSidebar() {
    const sidebar = document.getElementById('sidebar'); // Ensure there's a corresponding HTML element with this ID.
    sidebar.innerHTML = ''; // Clear existing sidebar content.

    markers.forEach(marker => {
        if (marker.getMap() !== null) { // Check if the marker is currently shown on the map.
            // Assuming infowindowContent is correctly assigned within each marker's context
            // This requires that infowindowContent logic is correctly handled elsewhere in the script
            const infoDiv = document.createElement('div');
            infoDiv.className = 'marker-info';
            infoDiv.innerHTML = marker.infowindowContent; // Use the infowindow content from the marker
...
// Function to update sidebar with marker info
function updateSidebar() {
    const sidebar = document.getElementById('sidebar'); // Ensure there's a corresponding HTML element with this ID.
    sidebar.innerHTML = ''; // Clear existing sidebar content.
    ...
    markers.forEach((marker) => {
        if (marker.getMap() !== null) { // Check if the marker is currently shown on the map.
            // Placeholder for extracting marker information. Adapt this line based on actual data handling.
            const markerInfo = { title: marker.title, description: marker.description }; // Mockup, replace with real data extraction logic
            const infoDiv = document.createElement('div');
            infoDiv.className = 'marker-info';
            infoDiv.innerHTML = `
...
// Example placeholder to call updateSidebar after marker operations. Integrate as needed.
// updateSidebar();
