...
function populateSidebarWithInfowindowContent() {
  ...
  markers.forEach((marker) => {
    if (marker.map && marker.infowindow) { // Check if the marker is currently on the map and has an infowindow
      const infowindowContent = safeGetContent(marker.infowindow); // Safely get the content
      if (infowindowContent) {
        const contentDiv = document.createElement('div');
        contentDiv.innerHTML = infowindowContent;

        // Extract the relevant parts of the content if they exist
        const header = contentDiv.querySelector('.infowindow-header') ? contentDiv.querySelector('.infowindow-header').innerText : '';
        const description = contentDiv.querySelector('.infowindow-description') ? contentDiv.querySelector('.infowindow-description').innerText : '';
