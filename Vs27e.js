...
function populateSidebarWithInfowindowContent() {
  ...
  markers.forEach((marker) => {
    // Assuming each marker has an 'infowindow' with the content that needs to be shown in the sidebar
    if (marker.map && marker.infowindow) { // Check if the marker is currently on the map and has an infowindow
      const infowindowContent = marker.infowindow.getContent(); // Get the HTML content of the infowindow
      // Create a div element to hold the content
      const contentDiv = document.createElement('div');
      contentDiv.innerHTML = infowindowContent;
      // Extract the relevant parts of the content
      const header = contentDiv.querySelector('.infowindow-header').innerText;
      const description = contentDiv.querySelector('.infowindow-description').innerText;
      const imageSrc = contentDiv.querySelector('img').src;
