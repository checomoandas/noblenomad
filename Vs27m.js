...
function safeGetContent(infowindow) {
  try {
    if (typeof infowindow.getContent === 'function') {
      return infowindow.getContent();
    }
  } catch (error) {
    console.error('Error getting content from infowindow:', error);
  }
...
document.addEventListener('DOMContentLoaded', function() {
  populateSidebarWithInfowindowContent(); // Call this function to populate the sidebar when the DOM is fully loaded
});
...
function initMap() {
    ...
    // Assign the Autocomplete object to a variable
    const autocomplete = new google.maps.places.Autocomplete(document.getElementById('startLocation'));
...
function populateSidebar() {
  ...
  markers.forEach((marker) => {
    // Assuming each marker has a property 'info' with the necessary data
    if (marker.map) { // Check if the marker is currently on the map
      // Create HTML content for each marker
      const markerContent = `
        <div class="marker-item" style="
          ...
          padding: 10px;
          border-bottom: 1px solid #ddd; /* For styling */
          margin-bottom: 10px;
        ">
          <div class="marker-image" style="width: 100%;">
            <img src="${marker.info.popupimage_url}" alt="Image" style="width: 100%; height: auto;">
          </div>
          <div class="marker-info" style="padding-top: 10px;">
            <h4>${marker.info.popup_header}</h4>
      ...
      sidebar.innerHTML += markerContent;
    }
  });
}

function populateSidebarWithInfowindowContent() {
    const sidebar = document.getElementById('sidebar');
    if (!sidebar) {
    ...
    markers.forEach((marker) => {
        if (marker.map && marker.infowindow) {
            const infowindowContent = safeGetContent(marker.infowindow);
            if (infowindowContent) {
                const contentDiv = document.createElement('div');
                contentDiv.innerHTML = infowindowContent;

                const header = contentDiv.querySelector('.infowindow-header') 
                               ? contentDiv.querySelector('.infowindow-header').innerText 
                               : '';
                const description = contentDiv.querySelector('.infowindow-description') 
                                    ? contentDiv.querySelector('.infowindow-description').innerText 
                                    : '';
                const image = contentDiv.querySelector('img');
                const imageSrc = image ? image.src : '';

                // Correcting the HTML string formation within the template literal
                const markerContent = `<div class="marker-item" style="display: flex; flex-direction: column; justify-content: space-between; padding: 10px; border-bottom: 1px solid #ddd; margin-bottom: 10px;">
                                            <div class="marker-image" style="width: 100%;">
